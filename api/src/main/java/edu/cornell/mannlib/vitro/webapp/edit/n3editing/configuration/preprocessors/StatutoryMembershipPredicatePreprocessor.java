...
public class StatutoryMembershipPredicatePreprocessor extends
		BaseEditSubmissionPreprocessorVTwo {

	protected static final Log log = LogFactory
			.getLog(StatutoryMembershipPredicatePreprocessor.class.getName());
	...
	private static String existingResearchTeamBase = "existingResearchTeam";
	private static String existingResearchUnitBase = "existingResearchUnit";
	...
	private static List<String> existingResearchTeamValues =  null;
	private static List<String> existingResearchUnitValues = null;
	...
	public StatutoryMembershipPredicatePreprocessor(EditConfigurationVTwo editConfig, WebappDaoFactory wadf) {
		super(editConfig);
	...
	private void copySubmissionValues() {
		...
		existingResearchTeamValues = getexistingResearchTeamValues();
		existingResearchUnitValues = getexistingResearchUnitValues();
	...
	private void getExistingConceptRelationships() {
		List<String> existingNarrowerURIs = getExistingNarrowerURIs(existingResearchUnitValues);
		List<String> existingBroaderURIs = getExistingBroaderURIs(existingResearchTeamValues);
		...
		if(existingNarrowerURIs.size() > 0) {
			urisFromForm.put("existingResearchUnit", existingNarrowerURIs);
		} else {
			//The original code for submission wouldn't put in a key if the values were null or size 0
			urisFromForm.remove("existingResearchUnit");
		}
		...
		this.existingResearchUnitValues = existingNarrowerURIs;
		if(existingBroaderURIs.size() > 0) {
			urisFromForm.put("existingResearchTeam", existingBroaderURIs);
		} else {
			urisFromForm.remove("existingResearchTeam");
		}
		this.existingResearchTeamValues = existingBroaderURIs;
	}
	...
	private List<String> getexistingResearchUnitValues() {
		return this.getJSONFormURIValues("existingResearchUnit");
	}

	private List<String> getexistingResearchTeamValues() {
		return this.getJSONFormURIValues("existingResearchTeam");
	}
	...
	private List<String> getJSONFormURIValues(String varName) {
		...
		if(uris.size() > 0) {
			...
			if(jsonString != null && !jsonString.isEmpty()) {
				ObjectNode jsonObject = (ObjectNode) JacksonUtils.parseJson(jsonString);
				//This should be an array
				if(jsonObject.isArray()) {
					 ArrayNode values = (ArrayNode) jsonObject.get(jsonString);
					 return JacksonUtils.jsonArrayToStrings(values);
				}
			}
		}
		return uris;
	...
	private void addInputsToSubmission(int numberConcepts) {
		...
		addexistingResearchTeamInputs(numberConcepts);
		addexistingResearchUnitInputs(numberConcepts);
	}
	...
	private void addexistingResearchTeamInputs(int numberConcepts) {
		int i;
		...
		if(this.existingResearchTeamValues.size() > 0 && this.existingResearchTeamValues.size() <= numberConcepts) {
			for(i = 0; i < numberConcepts; i++) {
				int suffix = i + 1;
				String existingResearchTeamInputName = existingResearchTeamBase + suffix;
				String broaderURIs = this.existingResearchTeamValues.get(i);
				if(broaderURIs != null && !broaderURIs.isEmpty()) {
					...
					//Add value for uri to form
					submission.addUriToForm(editConfiguration, existingResearchTeamInputName, broaderURISet);
				}
			}
		}

	}
	private void addexistingResearchUnitInputs(int numberConcepts) {
		int i;
		if(this.existingResearchUnitValues.size() > 0 && this.existingResearchUnitValues.size() <= numberConcepts) {
			for(i = 0; i < numberConcepts; i++) {
				int suffix = i + 1;
				String existingResearchUnitInputName = existingResearchUnitBase + suffix;
				String narrowerURIs = this.existingResearchUnitValues.get(i);
				if(narrowerURIs != null && !narrowerURIs.isEmpty()) {
					...
					//Add value for uri to form
					submission.addUriToForm(editConfiguration, existingResearchUnitInputName, narrowerURISet);
				}
			}
		}
	}
	...
	private void addFields(int numberConcepts) {
		...
		for (index = 1; index <= numberConcepts; index++) {
			...
			String existingResearchTeam = existingResearchTeamBase + suffix;
			String existingResearchUnit = existingResearchUnitBase + suffix;
			...
			addexistingResearchTeamField(existingResearchTeam);
			addexistingResearchUnitField(existingResearchUnit);
		}
	}
	...
	private void addexistingResearchUnitField(String existingResearchUnit) {
		editConfiguration.addField(new FieldVTwo().
				setName(existingResearchUnit));

	}

	private void addexistingResearchTeamField(String existingResearchTeam) {
		editConfiguration.addField(new FieldVTwo().
				setName(existingResearchTeam));
	...
	private void addLiteralsAndUrisOnForm(int numberTerms) {
		...
		for (index = 1; index <= numberTerms; index++) {
			...
			urisOnForm.add(existingResearchTeam);
			urisOnForm.add(existingResearchUnit);
	...
	private void addN3Optional(int numberConcepts) {
		...
		String existingResearchTeamVar = "?" + existingResearchTeamBase;
		String existingResearchUnitVar = "?" + existingResearchUnitBase;
		String prefixStr = "@prefix core: <http://vivoweb.org/ontology/core#> .";
		// First one already included so add new ones here
		...
		for (index = 1; index <= numberConcepts; index++) {
			...
			String existingResearchTeam = existingResearchTeamVar + suffix;
			String existingResearchUnit = existingResearchUnitVar + suffix;
			...
			String n3existingResearchTeam = prefixStr + node + " <" + this.SKOSNarrowerURI + "> " + existingResearchUnit + " ." +
					existingResearchUnit + " <" + this.SKOSBroaderURI + "> " + node + " .";
			String n3existingResearchUnit = prefixStr + node + " <" + this.SKOSBroaderURI + "> " + existingResearchTeam + " ." +
					existingResearchTeam + " <" + this.SKOSNarrowerURI + "> " + node + " .";
			...
			n3Optional.add(n3existingResearchTeam);
			n3Optional.add(n3existingResearchUnit);
