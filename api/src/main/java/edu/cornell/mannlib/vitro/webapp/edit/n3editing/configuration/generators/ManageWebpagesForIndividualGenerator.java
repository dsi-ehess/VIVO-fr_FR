...
public class ManageWebpagesForIndividualGenerator extends BaseEditConfigurationGenerator implements EditConfigurationGenerator {
    ...
    // Remove ?typeLabel, which has multiple values and causes links to obtain
    // multiple vivo:rank values when the ReorderController is run.  This causes
    // links to appear on the page multiple times.
    private static String WEBPAGE_QUERY = ""
            + "PREFIX core: <http://vivoweb.org/ontology/core#> \n"
            + "PREFIX vcard: <http://www.w3.org/2006/vcard/ns#> \n"
            + "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> \n"
            + "PREFIX vitro: <http://vitro.mannlib.cornell.edu/ns/vitro/0.7#> \n"
            + "SELECT DISTINCT ?vcard ?link ?url ?rank (group_concat(distinct ?linkLabel;separator=\"/\") as ?label) WHERE { \n"
            //HACK EHESS https://groups.google.com/forum/#!topic/vivo-tech/TLbfiiCMNCU
