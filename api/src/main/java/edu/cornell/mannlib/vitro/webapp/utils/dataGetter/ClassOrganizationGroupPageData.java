 ...
 */
public class ClassOrganizationGroupPageData extends DataGetterBase implements DataGetter{
	
    private static final Log log = LogFactory.getLog(ClassOrganizationGroupPageData.class);
     ...
     */
    public ClassOrganizationGroupPageData(VitroRequest vreq, Model displayModel, String dataGetterURI){
    ...
    public Map<String, Object> getData(Map<String, Object> pageData) {
        ...
        Map<String, List<VClass>> groupChildrenOrganization = new HashMap<>();
        for (VClass vClass : group) {
            if (!vClass.getURI().equals("http://xmlns.com/foaf/0.1/Organization")) {
                ...
                if (subClassStr != null && !subClassStr.isEmpty()){
                    ...
                    groupChildrenOrganization.put(vClass.getURI(), subClass);
                }
            }
        }
        ...
        data.put("vChildrenClassGroup", groupChildrenOrganization);  //may put null
        //This page level data getters tries to set its own template,
        // not all of the data getters need to do this.
        data.put("bodyTemplate", "page-classgroup-organization.ftl");
