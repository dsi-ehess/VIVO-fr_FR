...
<#-- this template is for adding a person's function to an organization -->
...
<h2>${titleVerb}&nbsp;${i18n().fnct_history_entry_for} ${editConfiguration.subjectName}</h2>
...
<section id="organizationHasFunctionHistory" role="region">

	<form id="organizationHasFunctionHistory" class="customForm noIE67" action="${submitUrl}"  role="add/edit function history">
	    <p>
	    ...
	    <p>
	        ...
	        <input size="30" type="text" id="functionTitle" name="functionTitle" value="${functionTitleValue}" />
	    </p>
	    ...
	    <select id="functionType" name="functionType">
	        ...
	        <#if (posnTypeOpts?keys)??>
		        <#list posnTypeOpts?keys as key>
                    <#if functionTypeValue?has_content && functionTypeValue = key>
...
$(document).ready(function(){
    orgHasFunctionUtils.onLoad('${blankSentinel}');
});
</script>
...
${scripts.add('<script type="text/javascript" src="${urls.base}/js/jquery-ui/js/jquery-ui-1.12.1.min.js"></script>',
             ...
             '<script type="text/javascript" src="${urls.base}/templates/freemarker/edit/forms/js/orgHasFunctionUtils.js"></script>',
