...
<#--HACK EHESS Point to "Bis" controller-->
<form action="${editConfiguration.deleteProcessingUrl}Bis" method="get">
<#--HACK EHESS END-->
...
<#--HACK EHESS-->
    <#if ["http://data.ehess.fr/ontology/vivo#AppartenanceStatutaire","http://data.ehess.fr/ontology/vivo#AppartenanceATitreDAssocie","http://data.ehess.fr/ontology/vivo#AppartenanceEnTantQuePostDoctorant"]?seq_contains(editConfiguration.rangeUri) && statement.composanteUnite??>
        <input type="hidden" name="deleteSecondaryObjectUri" value="${statement.uri("appartenanceEquipe")!}"/>
    </#if>
<#--HACK EHESS END-->
