    ...
    <nav role="navigation">
        <ul id="browse-classes">
            ...
            <#-- Only display vClasses with individuals -->
                ...
                <#if !vClass.URI?contains("http://vivoweb.org/ontology/core#FacultyMember") && !vClass.URI?contains("http://vivoweb.org/ontology/core#GraduateStudent")>
                    <#if (vClass.entityCount > 0) >
                        <li id="${vClassCamel}"><a href="#${vClassCamel}" title="${i18n().browse_all_in_class}" data-uri="${vClass.URI}">
                            <#if vClass.URI?contains("http://xmlns.com/foaf/0.1/Person")>
